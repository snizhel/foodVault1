...
{
    public partial class FrmUpdateOutput : Form
    {
        OutputManagement outputManagement = new OutputManagement();

        private readonly FrmFoodManagement frmFoodManagement1;

        public FrmUpdateOutput(FrmFoodManagement frmFoodManagement)
        {
            InitializeComponent();
            frmFoodManagement1 = frmFoodManagement;
        }

        private void FrmUpdateOutput_Load(object sender, EventArgs e)
        {
            loadOutputDetail();
        }
        ...
        private void btn_OutputSave_Click(object sender, EventArgs e)
        {
            ...
            {
                OutputId = FrmFoodManagement.outputId,
                DateOutput = dt_DataOutput.Value,
                TotalPrice = int.Parse(txt_TotalPriceOutput.Text.Trim()),
            };
            int result = outputManagement.UpdateOutput(output);
            if (result < 0)
            {
